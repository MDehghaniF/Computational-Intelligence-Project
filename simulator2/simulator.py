...
TRACK = pygame.image.load('./images/map.png').convert_alpha()   #convert to black and white
...
def additional_controller_init():
    center = []
    gas = []
...
def run():
    ...
    Runs the game
    """
    anf1 = fuzzy_controller_init()
    anf2 = additional_controller_init()
    music_mode="on"
    ...
    while True:
        ...
        if not car.crashed:
            ...
            output_of_fuzzy=rotate_fuzzy_system.decide(anf1,relative_left_dist,relative_right_dist)
            car.angle+=output_of_fuzzy

            gas_fuzzy_system=additional_controller.FuzzyGasController()

            car.update_position(gas_fuzzy_system.decide(anf2,center_dist))
            car.display_car()
